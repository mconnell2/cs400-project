...
public class Meal {
  ...
  /*
   * removes a food item from the meal from the specific index.
   * If the food item to remove does not exist, the meal is not changed.
   */
  public void removeFoodItem(int index) {
    if ((index >= 0) && index < foodItemList.size()) foodItemList.remove(index);
  ...
  /*
   * Returns a HashMap of nutrients (key, String) and grams (value, Double) for all food items
   * in the meal
   */
  /**
  ...
  public HashMap<String, Double> analyzeMeal() {
    ...
    for (FoodItem foodItem: foodItemList) {
      ...
      for (String nutrient: foodNutrients.keySet()) {
        ...
        if (currentMealTotal == null) mealNutrients.put(nutrient, foodNutrients.get(nutrient));
        else mealNutrients.put(nutrient, currentMealTotal + foodNutrients.get(nutrient));
      }
    }
  ...
  public static void main(String[] args) {
    ...
    fi1.addNutrient("fat", 0.1); fi1.addNutrient("carbs", 0.1);
    fi3.addNutrient("fat", 1003.1);
    fi2.addNutrient("fat", 10.0); fi2.addNutrient("fat", 12.0); fi2.addNutrient("carbs", 16.0);
